...
if (isset($_POST['csrf_token'])) {
    $insert = $conn->prepare('UPDATE admin_hub_details SET otp = ? WHERE email = ?');
    $insert->bind_param('is', $otp, $email);
    $insert->execute();
}
$select = $conn->prepare('SELECT * FROM admin_hub_details WHERE email = ?');
